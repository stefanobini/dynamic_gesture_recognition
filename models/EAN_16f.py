...
class ShiftModule(nn.Module):
    def __init__(self, input_channels, n_segment=8,n_div=8, mode='shift'):
        ...
        self.conv = nn.Conv1d(self.fold_div*self.fold, self.fold_div*self.fold, kernel_size=3, padding=1, groups=self.fold_div*self.fold, bias=False)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, block2, layers, num_segments, flow_estimation, num_classes=1000, zero_init_residual=False, sample_size=112):
    ...
    # Resize the input, in the TSN module it is reshaped so, in their implementation they reshape back
    def d2_to_d3(self, x, t=16):
        print('ResNet d2_to_d3, x size is {}'.format(x.size()))
        bt, c, h, w = x.size()
        b = bt//t
        return x.reshape(b,t,c,h,w).transpose(1,2)
    ...
    def forward(self, x, temperature=0):
        x.permute(0, 2, 1, 3, 4)
        print('INPUT: ', x.size())
        print('STRANGE DIMENSIONs: {}, {}'.format((-1, self.num_segments,3), x.size()[-2:]))
        x = x.view((-1, self.num_segments,3) + x.size()[-2:])
        print('VIEW: ', x.size())
        x = x.transpose(1,2).contiguous()
        print('CONTIGUOUS: ', x.size())
        b,c,t,h,w = x.size()
...
def resnet101(pretrained=False, shift='TSM', sample_duration = 16, flow_estimation=0, **kwargs):
    """Constructs a ResNet-101 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    if (shift =='TSM'):    
        model = ResNet(Bottleneck, Bottleneck, [3, 4, 23, 3], num_segments=sample_duration , flow_estimation=flow_estimation, **kwargs)          
    if pretrained:
